		...
		<nav>
			<li class="single-entry">
				...
				<?php elseif($registration_enabled): ?>
					<?php echo sprintf('<a href="%1$s">' . __('Login', 'aurum') . '</a> or <a href="%1$s">' . __('Register', 'aurum') . '</a>', $account_link); ?>
				<?php else: ?>
...
function share_story_network_link($network, $id, $simptips = true)
{
	...
	$networks = array(
		...
		'tlr' => array(
			'url'		=> 'http://www.tumblr.com/share/link?url=' . esc_attr( get_permalink() ) . '&name=' . esc_attr( get_the_title() ) . '&description=' . esc_attr( get_the_excerpt() ),
