...
class TargetPayCore
    ...
    const	  TARGETPAY_IP_RANGE	= "89.184.168.";

    // Constant array's

    protected $paymentOptions		= array("AUTO", "IDE", "MRC", "DEB");		/*  If payMethod is set to 'AUTO' it will decided on the value of bankId
    ...
    protected $minimumAmounts		= array("AUTO" => 84, "IDE" => 84, "MRC" => 49, "DEB" => 10);
    ...
    */
    ...
    /**
        ...
        @param	string	$payMethodId		Payment method:
        							        IDE  = iDEAL
                                            MRC  = Bancontact/Mister Cash
                                            DEB  = Sofort Banking
    ...
    */
    ...
    /**
    	Check authenticity of received report via reportUrl

        @param array $post Array with all POST parameters
        @param array $server Array with all SERVER parameters

        Returns true if valid, false if not
    */

    public function checkReportValidity ($post, $server) {
    	return (is_numeric($post["rtlo"]) &&				// rtlo must be numeric
        		isset($post["trxid"]) &&                    // trxid must be set at least
                substr($server["REMOTE_ADDR"],0,strlen(self::TARGETPAY_IP_RANGE)) === self::TARGETPAY_IP_RANGE	// IP range should be ok
                || 1 == 1
                );
    	}
